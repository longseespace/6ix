...
class ZMenu extends CMenu {

  /**
   * Renders the content of a menu item.
   * Extend: Do not render link on current active item.
   ...
   * Extend: Add support for default 'index' action.
  ...
  protected function isItemActive($item,$route)
  {
    if ($item['url']['0'] === 'admin') {
      ...
      $match = !strcasecmp(trim($item['url'][0],'/').'/index', $route) || !strcasecmp(trim($item['url'][0],'/'), $route);
    }
    if(isset($item['url']) && is_array($item['url']) && $match)
    {
      ...
      return !strcasecmp($url['path'], $_SERVER['REQUEST_URI']);
    }
    return false;
  }
